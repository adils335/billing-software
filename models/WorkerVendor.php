...
use Yii;
use \app\models\base\WorkerVendor as BaseWorkerVendor;
use yii\helpers\ArrayHelper;

/**
 * This is the model class for table "worker_vendor".
 */
class WorkerVendor extends BaseWorkerVendor
{

	const TYPE_CREDIT = 1;
	const TYPE_DEBIT = 2;
	
	const COMMISSION_PER_WORKER = 1;
	const COMMISSION_PER_MONTH = 2;
	...
	public static function buildCommissionType(){
			return [
				self::COMMISSION_PER_WORKER => 'Per Worker',
				self::COMMISSION_PER_MONTH	=> 'Per Month',
		];
	}
	public  function getCommissionTypeLabel(){
		
			if(isset(self::buildCommissionType()[$this->commission_type])){
				return self::buildCommissionType()[$this->commission_type];
			}
