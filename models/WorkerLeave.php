...
use Yii;
use \app\models\base\WorkerLeave as BaseWorkerLeave;
use yii\helpers\ArrayHelper;

/**
 * This is the model class for table "worker_leave".
 */
class WorkerLeave extends BaseWorkerLeave
{
    ...
    public static function resetLeave($id){
        ...
        Self::UpdateAll(['leave'=>Null],['worker_id'=>$id,'month'=>$month]);
    }
    ...
    public static function filter_leave($id,$start_month,$end_month){
        ...
        $model = Self::find()->where(['BETWEEN','month',$start_month,$end_month])->orderBy(['month'=>SORT_DESC,'worker_id'=>SORT_DESC]);
        if($id){
            $model = $model->andWhere(['worker_id'=>$id]);
        }
        return $model;
    }
