<?php

namespace app\models;

use Yii;
use \app\models\base\StoreIndents as BaseStoreIndents;
use yii\helpers\ArrayHelper;

/**
 * This is the model class for table "store_indents".
 */
class StoreIndents extends BaseStoreIndents
{
    const STATUS_ACTIVE = 1;
	const STATUS_DEACTIVE = 2;
	const STATUS_DELETE = 3;

    public function behaviors()
    {
        return ArrayHelper::merge(
            parent::behaviors(),
            [
                # custom behaviors
            ]
        );
    }

    public function rules()
    {
        return ArrayHelper::merge(
            parent::rules(),
            [
                # custom validation rules
            ]
        );
    }
    

    public static function buildStatus(){
        return [
            self::STATUS_DEACTIVE =>'Deactive',
            self::STATUS_ACTIVE	=>'Active',
            self::STATUS_DELETE	=>'Delete',
        ];
    }

    public function getStatusLabel(){
        if(isset(self::buildStatus()[$this->status])){
            return self::buildStatus()[$this->status];
        }
    }
    public function getIndentNo(){
        $indent_no = '';

        $last_id = StoreIndents::find()->max('id');
        $id = $this->company_id;
        $session = $this->session;
        $last_id++;
        $this->indent_no = $id."/".$session."/".$last_id;
    }
}
